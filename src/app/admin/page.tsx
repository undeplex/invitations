...
export default function AdminLinkGenerator() {
  ...
  const [filterType, setFilterType] = useState<'all' | 'single' | 'couple' | 'registered' | 'not-registered'>('all')
  const [currentPage, setCurrentPage] = useState(1)
  const [linkType, setLinkType] = useState<'normal' | 'quick'>('normal')
  ...
  const generateLink = async () => {
    ...
    try {
      ...
      // Toujours utiliser /qr-form comme URL
      const params = new URLSearchParams({
        id,
        couple: String(isCouple),
        sig: signature,
        forceForm: linkType === 'quick' ? 'true' : 'false' // Ajouter forceForm pour les liens rapides
      })
      ...
      // ENSUITE, si c'est un lien rapide, créer l'enregistrement dans wedding_invitations
      if (linkType === 'quick') {
        ...
        // For normal links, check registration status
        isRegistered = await checkRegistrationStatus(id)
      }
      ...
      setIsCouple(false)
      setLinkType('normal')
      showNotification(`Lien ${linkType === 'normal' ? 'normal' : 'rapide'} généré avec succès!`, 'success')
      setCurrentPage(1)
    } catch (error) {
      console.error('Error generating link:', error)
        ...
        <div className="max-w-7xl mx-auto p-4 pt-6">
          ...
          <div className="bg-white p-6 lg:w-10/12 rounded-xl shadow-sm mb-6">
            <div className="grid grid-cols-1 lg:grid-cols-6 gap-4">
              ...
              <div className="lg:col-span-1">
                <select
                  value={linkType}
                  onChange={(e) => setLinkType(e.target.value as 'normal' | 'quick')}
                  className="w-full text-sm px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  disabled={isLoading}
                >
                  <option value="normal">Lien Normal</option>
                  <option value="quick">Lien Rapide</option>
                </select>
              </div>
              ...
              <div className="lg:col-span-1">
                <button
                  ...
                  disabled={isLoading || !name.trim()}
                  className={`w-full ${
                    isLoading || !name.trim() ? 'bg-blue-400' : 
                    linkType === 'quick' ? 'bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700' : 
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      {linkType === 'quick' ? <FiZap size={18} /> : <FiPlus size={18} />}
                      {linkType === 'quick' ? 'Generer Rapide' : 'Generer'}
                    </>
                  )}
                </button>
              </div>
          ...
          {links.length > 0 && (
            <div className="bg-white p-6 rounded-xl shadow-sm">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
                <h2 className="text-xl font-semibold text-blue-800">
                  Liens Générés ({filteredAndSortedLinks.length}/{links.length})
                </h2>
                <button
                  onClick={() => {
                    if (confirm('Clear all links? This cannot be undone.')) {
